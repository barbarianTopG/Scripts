...
local bobTime = 0;
local baseCamOffset = Vector3.zero
...
renderConnection = RunService.RenderStepped:Connect(function(deltaTime)
	...
	if isMoving then
		...
		if sprinting then
			local shakeOffset = Vector3.new(math.random(-5, 5) * 0.01, math.random(-3, 3) * 0.01, 0)
			baseCamOffset = baseCamOffset + shakeOffset
		end
	else
		baseCamOffset = baseCamOffset:Lerp(Vector3.zero, 0.2)
		bobTime = 0
	end;
