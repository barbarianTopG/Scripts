...
G2L["Sprint_2"]["Font"] = Enum.Font.Cartoon
...
local function setSprinting(state)
    if state and stamina > 0 then
        ...
        TweenService:Create(sprintBtn, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(0, 100, 200)}):Play()
    else
        sprinting = false
...
rsConn = RunService.RenderStepped:Connect(function(dt)
    ...
    TweenService:Create(staminaBar, TweenInfo.new(0.1), {
        Size = UDim2.new(stamina/maxStamina, 0, 0.5, 0)
    }):Play()
