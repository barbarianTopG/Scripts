...
-- Add message function with improved UI (defined before use)
local function AddMessage(msg, player)
...
-- Listen for messages from other players (fixed implementation)
...
-- Connect to the message event properly
if TextChatService.TextChannels.RBXGeneral then
    TextChatService.TextChannels.RBXGeneral.OnIncomingMessage:Connect(onIncomingMessage)
else
    -- Fallback if RBXGeneral isn't available yet
    local connection
    connection = TextChatService:WaitForChild("TextChannels").ChildAdded:Connect(function(child)
        if child.Name == "RBXGeneral" then
            child.OnIncomingMessage:Connect(onIncomingMessage)
