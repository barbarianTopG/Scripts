...
local function createMap()
    ...
    accretionDisk.Size = Vector3.new(1, CONFIG.BlackHoleSize * 3, CONFIG.BlackHoleSize * 3)
    ...
    accretionDisk.Orientation = Vector3.new(0, 0, 90)
    ...
    for i = 1, CONFIG.UFOCount do
        ...
        local ufoRing = Instance.new("Part")
        ufoRing.Name = "UFORing_" .. i
        ufoRing.Shape = Enum.PartType.Cylinder
        ufoRing.Size = Vector3.new(1, ufo.Size.X * 1.5, ufo.Size.X * 1.5)
        ufoRing.Position = ufo.Position
        ufoRing.Anchored = true
        ufoRing.Material = Enum.Material.Neon
        ufoRing.Color = ufo.Color
        ufoRing.Transparency = 0.3
        ufoRing.CanCollide = false
        ufoRing.Parent = island
        ufoRing.Orientation = Vector3.new(0, 0, 90)
        ...
        task.spawn(function()
            ...
            while ufo.Parent and ufoDome.Parent do
                ...
                ufoRing.CFrame = ufoRing.CFrame * CFrame.Angles(0, math.rad(0.5), 0)
    ...
    for i, data in ipairs(CONFIG.PlanetData) do
        ...
        if data.Spin then
            task.spawn(function()
                while planet.Parent do
                    planet.CFrame = planet.CFrame * CFrame.Angles(0, math.rad(0.5), 0)
                    task.wait(0.03)
                end
            end)
        end
        ...
        if data.Ring then
            ...
            ring.Size = Vector3.new(1, data.Size * data.RingSize, data.Size * data.RingSize)
            ...
            task.spawn(function()
                while ring.Parent do
                    ring.CFrame = planet.CFrame * CFrame.Angles(0, math.rad(0.3), 0)
                    task.wait(0.03)
                end
            end)
        end
    ...
    for i = 1, CONFIG.AsteroidCount do
        ...
        asteroid.Color = Color3.fromRGB(math.random(100, 200), math.random(100, 200), math.random(100, 200))
        asteroid.Reflectance = 0.2
    ...
    for i = 1, CONFIG.StarCount do
        ...
        star.Color = Color3.fromRGB(math.random(200, 255), math.random(200, 255), math.random(200, 255))
    ...
    table.insert(specialEffects, skybox)
    
    local aura = Instance.new("Part")
    aura.Name = "CosmicAura"
    aura.Size = CONFIG.AuraSize
    aura.Position = platform.Position
    aura.Anchored = true
    aura.Transparency = 1
    ...
    table.insert(specialEffects, auraParticles)
...
EffectsToggle.MouseButton1Click:Connect(function()
    ...
    if isEffectsEnabled then
        ...
        if not workspace:FindFirstChild("StellarNexusIsland") then
            createMap()
        end
    else
...
CloseButton.MouseButton1Click:Connect(function()
    ...
    if not isCustomLighting then
        applyLighting(originalLighting)
    end
    if not isMusicEnabled then
        toggleMusic(false)
    end
    if not isEffectsEnabled then
        for _, obj in pairs(workspace:GetChildren()) do
            if obj.Name == "StellarNexusIsland" then
                obj:Destroy()
            end
